...
**`sakai-tasks-mcp`** は、大学 LMS「Sakai」とデスクトップ AI アシスタント（Claude Desktop, Cursor 等）を安全に接続するデスクトップ MCP サーバーです。課題・小テスト・お知らせ・講義資料などのタスク情報を、**Model Context Protocol (MCP)** を通じてオンデマンドで AI へ提供します。

常駐プロセスやバックグラウンドサービスは不要です。AI クライアントからサブプロセスとして起動され、標準入出力（stdio）を介して JSON-RPC 通信を行います。学生は AI との自然言語対話を通じて、直近の締切や重要連絡を瞬時に把握できます。
...
1. **標準入出力 (stdio) の厳格な保護**:
   * MCP プロトコルは標準入出力を JSON-RPC 通信チャネルとして占有します。`print()` などによる標準出力への書き込みは通信プロトコルを破壊するため**厳禁**です。すべてのログ・デバッグ出力は `sys.stderr` または `logging`（stderr ハンドラ）に統一します。
2. **セキュア・バイ・デフォルト (Secure by Default)**:
   * 講義ごとの AI 利用権限は、未設定の講義や新規追加された講義において、すべて **`TEXT_ONLY`（強: ファイルダウンロード禁止）** をデフォルトとします。教員の講義資料ファイルが意図せず外部 AI に送信される事故を構造的に排除します。
3. **高凝集・疎結合 & 純粋関数設計**:
   * レスポンスのパース（`src/client/parsers/`）、セキュリティフィルタ（`src/policy/policy_filter.py`）、UI データ構築（`src/gui/data_builder.py`）は、外部状態や GUI に依存しない**純粋関数**として実装し、単体テストを容易に行える設計とします。
...
2. **セッション検証**: `SessionManager` がローカルに保存された Cookie の有効性を確認し、呼び出し元に返却します。
3. **データ取得 & パース**: `SakaiClient` が Sakai Direct REST API から生データを並行取得し、パーサーが共通モデル `list[SakaiTask]` へ正規化します。
4. **ポリシーフィルタ**: `policy_filter.py` が講義ごとの権限ポリシーを適用し、許可されていないデータのマスキングや除外を行います。
5. **返却**: 安全に整形された構造化データを FastMCP 経由で AI に返却し、AI がユーザー向けの回答文を生成します。
...
WebView2 (pywebview) は OS のメインスレッドで動作する必要があります。そのため、非同期イベントループ内からは直接画面を開けません。本システムでは **別プロセス (`server.py --login`) を起動してメインスレッドで認証を完結** させることで、この制約を安全に回避しています。
...
* **0.5 秒の自動素通り認証**: 固定プロファイル（`WEBVIEW_DATA_DIR`）に保存された大学 SSO（IdP / Microsoft 365 等）の永続 Cookie を利用するため、画面を表示することなくバックグラウンドでセッションを高速に更新します（※SSO Cookie 自体が存在しない初期実行時のみ、手動ログイン用ウィンドウが表示されます）。
* **暗号化保存**: 抽出された Cookie は、暗号化キーを OS 資格情報ストア（`keyring`）で管理し、共通鍵暗号（`Fernet`）により `session.enc` へ安全に保存します。
...
設定中も AI との対話を継続できるよう、**設定画面はポップアップとして別プロセス（`--settings`）で起動** します。
...
* **事前データ構築**: ウィンドウ表示前に `build_settings_ui_data()` が非同期で全講義一覧とお気に入り情報を取得し、既存設定とマージした `SettingsUIData` を構築します。画面表示後のチラつきやデータ取得待ちの発生を防ぎます。
* **動的設定反映**: 設定を保存すると、親プロセス側の `Config.get_course_policy()` がファイルの更新日時（mtime）の変化を自動検知し、サーバーを再起動することなく最新設定を即座に反映します。
...
| :--- | :--- | :--- | :--- |
| **`list_courses`** | `favorites_only: bool = True` | `filter_courses` | 履修・所属講義の一覧を取得。お気に入り講義を優先して取得し、お気に入りが未設定の場合は全講義を取得。 |
| **`get_assignments`** | `site_id: str \| None = None`<br/>`assignment_id: str \| None = None`<br/>`favorites_only: bool = True`<br/>`include_details: bool = True` | `filter_tasks` | 課題一覧、または個別課題の詳細（指示文・提出期限・添付ファイル情報）を取得。 |
| **`get_quizzes`** | `site_id: str \| None = None`<br/>`favorites_only: bool = True` | `filter_tasks` | 小テスト・クイズの一覧を取得。対象講義の SAMIGO API から並行取得して統合。※Sakai の仕様上、すべてのテストが未受験判定となります。 |
| **`get_upcoming_deadlines`** | `days: int \| None = None`<br/>`favorites_only: bool = True` | `filter_tasks` | 直近の未提出課題とテストを締切順にまとめて取得。日数が未指定の場合は `Config.DEFAULT_DEADLINE_DAYS` (30日) を適用。**締切を尋ねられた際の最優先ツール**。 |
| **`get_announcements`** | `site_id: str \| None = None`<br/>`announcement_id: str \| None = None`<br/>`n: int \| None = None`<br/>`favorites_only: bool = True`<br/>`include_details: bool = True` | `filter_announcements` | お知らせ・連絡事項の一覧または詳細を取得。`n` が未指定の場合は `Config.DEFAULT_ANNOUNCEMENT_LIMIT` (7件) を適用。 |
| **`get_calendar_events`** | `site_id: str \| None = None`<br/>`start_date: str \| None = None`<br/>`end_date: str \| None = None`<br/>`event_type: str \| None = None` | `filter_calendar_events` | 講義カレンダーの予定やイベント一覧を取得。日付は ISO 8601 形式の文字列で指定。 |
...
| **`get_course_dashboard`** | `site_id: str` (必須) | `filter_dashboard` | 指定講義の課題・テスト・お知らせ・資料を並行して一度に取得し、統合ダッシュボードモデル（`CourseDashboard`）を返却。 |
| **`download_material`** | `url: str` (必須)<br/>`save_path: str` (必須) | `check_download_allowed_by_url` | 講義資料や課題の添付ファイルを指定パスへダウンロード保存。講義ポリシー違反時はダウンロードを拒否。 |
| **`open_settings`** | *(引数なし)* | *(不要)* | ユーザー設定画面 (WebView GUI) を別プロセスで起動。stdio をブロックしないため、設定中も対話を継続可能。 |
| **`get_settings`** | *(引数なし)* | *(不要)* | 現在の設定一覧（接続ホスト名、講義別ポリシーなど）を取得。ポリシー制限の理由を説明する際などに利用。 |
| **`check_auth_status`** | *(引数なし)* | *(不要)* | セッションの有効性を確認（`userEid` を検査）。失効時は再ログイン案内を返し、強制的な画面表示は行わない。 |
...
* **AI の挙動**: 講義一覧から対象の講義 ID を特定したあと、`get_course_dashboard(site_id="2024_algo_01")` を呼び出します。
* **返却データ**: 該当講義の課題、テスト、最新お知らせ、講義資料が一括で返却されます。個別のツールを何度も往復して呼び出す必要がなく、少ない待ち時間で総合ダッシュボードを提示できます。

#### ユースケース 3: 緊急連絡・休講情報の確認
* **学生の発言**: 「最新の休講情報や教室変更の連絡はある？」
* **AI の挙動**: `get_announcements(n=5, include_details=True)` を呼び出します。
* **返却データ**: 履修している全講義の最新お知らせ。AI はタイトルや本文から「休講」「教室変更」「緊急」といったキーワードを抽出して要約します。
...
* **AI の挙動**: 対象講義のポリシーが `TEXT_ONLY`（デフォルト）の場合、`download_material` は拒否されます。AI は `get_settings` で現在の設定を確認したうえで、「現在の講義ポリシーが『ファイルDL禁止（TEXT_ONLY）』に設定されています。設定画面を開いてポリシーを変更しますか？」と案内し、`open_settings()` を呼び出して設定画面を起動します。
...
システム全体で共通利用される Pydantic v2 のデータモデル群です。全フィールドに `description` を付与し、AI が MCP 経由で各項目の意味を正確に解釈できるようにしています。
...
* **防御的設計**: 将来の Sakai バージョンアップに伴う未知のフィールド追加に対応するため、`extra = "ignore"` を標準としています。日時文字列はすべて ISO 8601（JST）形式に統一しています。
...
* **`cookie_storage.py`**:
  ...
  * 暗号化キーを `keyring` 経由で OS 資格情報ストア（Windows: DPAPI / macOS: Keychain）に格納し、`Fernet`（AES-128-CBC + HMAC-SHA256）で Cookie データを暗号化します。
* **`session_manager.py`**:
...
* **`webview_auth.py`**:
  * OS メインスレッドで Edge WebView2 / WebKit を起動し、大学 SSO の永続 Cookie を利用して画面を表示しない高速な自動再認証を実行します。
...
* **Singleflight 機構 & インメモリキャッシュ**:
  * 同一エンドポイントへの重複リクエスト合流制御（Singleflight）と TTL キャッシュ（既定 300 秒）を備え、Sakai サーバーへの過度な負荷集中を防ぎます。
...
2. **`TEXT_ONLY` (強 / ★デフォルト)**:
   * テキストのみ。課題指示文やお知らせ本文は取得可能だが、PDF・Office 文書等のファイル実体ダウンロードを禁止。講義資料一覧は空リストを返し、課題の添付ファイル一覧もマスクします。
3. **`SCHEDULE_ONLY` (弱)**:
   * 締切・予定のみ。課題の指示文やお知らせ本文を `"[講義ポリシーにより非表示]"` にマスキングし、ファイルダウンロードも禁止します。タイトル・締切日時・ステータス・カレンダー予定のみ利用できます。
4. **`BLOCKED` (無効)**:
   * 完全除外。講義一覧を含め、対象講義の全データ（課題・テスト・連絡・予定）を AI から完全に隠蔽・除外します。
...
* **設定管理 (`src/config.py`)**:
  * `platformdirs.user_data_dir("sakai-tasks-mcp") / "config.json"` に設定を保存します。
  * `sakai_host`（接続ドメイン名）、`policies`（講義別ポリシー）、`default_deadline_days`（締切日数）、`default_announcement_limit`（お知らせ件数）を管理します。
  * `Config.get_course_policy(site_id)` は設定ファイルの更新日時（mtime）を監視しており、別プロセスでの GUI 設定保存を即座に検知して再読み込みします。
* **GUI の 3 ファイル責務分離 (`src/gui/`)**:
  1. **`data_builder.py`**: Sakai から講義一覧を取得し、既存設定とマージしてお気に入り順にソートした単一 JSON モデル（`SettingsUIData`）を非同期で構築します（GUI に依存しない純粋関数）。
  2. **`api.py`**: JavaScript と Python 間の通信ブリッジを提供します（`get_initial_data()`, `save_settings(json)`）。
  3. **`settings_window.py`**: 事前構築したデータをバインドし、WebView ウィンドウを単一で起動するコントローラーです。
...
* `--login` 指定時: メインスレッドで `authenticate_via_webview()` を実行し、Cookie を保存して即座に終了します（終了コード 0）。
* `--settings` 指定時: 設定画面 GUI（`show_settings_window()`）を起動して終了します。
* `--setup` 指定時: 初回の大学プリセット選択ダイアログを起動して終了します。
* 通常起動時: `Config.init()` を実行後、`mcp.run(transport="stdio")` により stdio 通信を開始します。
...
#### 単一 exe 配布を避ける理由
* **アンチウイルス誤検知 (False Positive) の完全回避**: PyInstaller などによる単一 exe 形式は、Windows Defender や大学推奨のセキュリティソフトに誤検知されやすいため、Python 公式の署名済み Embeddable Python ランタイムをそのまま同梱して配布します。
...
モジュール間の疎結合および純粋関数設計により、**外部 Sakai サーバーとの実通信や GUI 画面の起動を伴わずに、ローカル環境だけで自動テストを完結** できます。
...
| **`tests/test_server.py`** | `src/server.py` | stdio 経由での FastMCP ツールの呼び出しと、正しい JSON Schema レスポンスの返却。 |
...
* **stdio 汚染厳禁**:
  * 開発中のデバッグであっても、`print()` を残してはなりません。JSON-RPC 通信が破損し、MCP クライアント側でツールの呼び出しがタイムアウトや構文エラーを起こす原因となります。ログ出力には必ず `sys.stderr.write()` または `logging` を使用してください。
* **純粋関数の維持**:
  * パーサー、ポリシーフィルタ、UI データ構築モジュールには外部への副作用（直接のネットワーク通信や GUI 描画）を持ち込まず、入力に対する純粋な変換ロジックを保ちます。
